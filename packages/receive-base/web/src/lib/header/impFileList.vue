...
export default {
  ...
  name: 'impFilelist',
  data() {
    return {selectData: [], expSchemaId: '', sourceCode: '',fileList:[],}
  },
  methods: {
    //显示弹出框
    showDialog() {
    ...
    beforeUpload(file) {
      ...
      if (['xml', 'xlsx', 'xls', 'zip', 'dbf', 'accdb'].indexOf(FileExt.toLowerCase()) === -1) {
        this.$message({
          type: 'warning',
          message: '请上传符合后缀名的附件！'
        });
