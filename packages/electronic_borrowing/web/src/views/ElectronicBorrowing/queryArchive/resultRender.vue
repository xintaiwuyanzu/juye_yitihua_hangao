<template>
  <el-card shadow="hover" class="search_result_item">
    <section class="result_title">
      <el-tooltip>
        <span slot="content" v-html="item.TITLE" style="font-weight: 700"/>
      ...
      <section class="result_btns" style="float: right;margin-top: -4px;margin-right: 10px">
<!--        <el-button type="primary" size="mini" @click="addDetail()">添加</el-button>-->
        <slot/>
...
export default {
  ...
  computed: {
    href() {
      ...
      return '#/ElectronicBorrowing/metadataAndFile?' + qs.stringify(query)
    }
  },
  methods: {
    $init() {
