  ...
  methods: {
    ...
    async getManagementHistory() {
      if (this.type === 'history') {
        const {data} = await this.$http.post('/managementHistory/detail', {id: this.managementHistoryId})
        if (data && data.success) {
          this.managementHistory = data.data
        }
      } else if (this.type === 'current') {
        const {data} = await this.$http.post('/management/detail', {id: this.managementHistoryId})
        if (data && data.success) {
          this.managementHistory = data.data
        }
      }
