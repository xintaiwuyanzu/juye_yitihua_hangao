...
<script>
  export default {
    ...
    methods: {
      ...
      async getTag() {
        const {data} = await this.$post('/factTag/detail', {id: this.$route.query.id})
      ...
      async save() {
        this.loading = true
        const {data} = await this.$post('/factTag/update', this.tag)
        if (data && data.success) {
