<template>
  <table-index title="离线接收详情" :fields="fields" path="receive/offline/detail" :insert="false"
    ...
    <template v-slot:table-$btns="scope">
      ...
      <el-button type="text" size="mini" width="60" @click="toTestReport(scope.row)">检测报告</el-button>
      <el-button type="text" size="mini" width="30" @click="deleteDetail(scope.row)" v-show="showEditButton">删除
      </el-button>
    </template>
    <el-table-column prop="archiveCode" after="fondName" label="档号" show-overflow-tooltip align="center" width="180">
      <template v-slot="{row}">
        <el-button type="text" @click="routeDetail( row)">{{ row.archiveCode }}</el-button>
      </template>
    </el-table-column>
    <!--    <el-table-column prop="fourDetection" label="检测结果" show-overflow-tooltip align="center"
                         width="100px">
          <template v-slot="scope">
            <el-link type="primary" @click="showDialog(scope.row)">{{ scope.row.fourDetection }}</el-link>
          </template>
        </el-table-column>-->
    <el-dialog width="80%" title="检测报告" :visible.sync="dialogShow" append-to-body>
      <el-table :data="data" border class="table-container" v-loading="dialogLoading">
        ...
        <el-table-column prop="testName" label="检测项" header-align="center" align="center" show-overflow-tooltip/>
...
export default {
  ...
  data() {
    return {
      ...
      //新格式
      fields: [
        {prop: 'fondName', label: '全宗', align: 'left', width: '100'},
  ...
  methods: {
    async $init() {
      this.$http.post('receive/offline/detail', {id: this.batchId})
          .then(({data}) => {
            if (data.success) {
              this.offLineRecord = data.data
              //判断全部挂接完毕和四性检测全部通过才可显示入库按钮
              if (data.data.hookNum == data.data.hookSuccessNum && data.data.testStatus === '1' && (data.data.status != 5 && data.data.status != 6)) {
                //this.showRk = true
              }
    ...
    async editForm(row) {
      ...
      if (data.success) {
        this.form = data.data
        this.defaultParams = {formDefinitionId: row.formDefinitionId, categoryId: row.categoryId, detailId: row.id}
        this.editDialogShow = true
      }
    },
    //编辑目录保存
    async save() {
      if (this.form.status_info !== 'RECEIVE') {
