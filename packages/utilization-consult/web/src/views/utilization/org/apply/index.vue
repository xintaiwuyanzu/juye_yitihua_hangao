<template>
  <table-index path="utilization/consult" :fields="columns" :edit="false" :delete="false" :insert="false" :defaultSearchForm="defaultSearchForm"
    ...
    <template v-slot:search-$btns>
      <el-button type="primary" @click="routeAdd">申请</el-button>
      <el-button type="primary" v-if="hasRole('aaa')">推送原文</el-button>
    </template>
    <template v-slot:table-$btns="{row}">
      <el-button type="text" @click="routeDetail(row)" width="20">详情</el-button>
    </template>
  </table-index>
</template>
<script>
  ...
  data() {
    return {
      columns: {
        ...
        receiveOrgId: {label: '受理单位', search: true, width: 120, fieldType:'select',url:'organise/getAllDepartment',params:{page:false},labelKey:'organiseName'},
      ...
      defaultSearchForm:{sourceType:'org',createOrgId:'true'}
    }
  },
  setup() {
