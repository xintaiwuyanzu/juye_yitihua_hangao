<template>
  <table-index path="utilization/consult" :fields="columns" :edit="false" :delete="false" :insert="false" :defaultSearchForm="defaultSearchForm"
               title="馆际查档审批">
    ...
    <template v-slot:table-$btns="{row}">
      <el-button type="text" v-show="row.status!='2'" @click="routeSearch(row)" width="40">查档</el-button>
...
export default {
  ...
  data() {
    return {
      columns: {
        ...
        userName: {label: '申请单位', search: false, width: 120, fieldType:'select',url:'organise/getAllDepartment',params:{page:false},labelKey:'organiseName'},
        phone: {label: '联系电话', width: 100, role: consultRole},
        ...
        other: {label: '其他'},
        status: {label: '状态', width: 80, fieldType:'select',component: 'tag',
          mapper:[{
            id: '0',
            label: '待查档'
          },{
      ...
      defaultSearchForm:{sourceType:'org',receiveOrgId:'true'}
    }
  },
  setup() {
  ...
  methods: {
    ...
    routeSearch(row) {
      if(row.status==0){
        row.status = 1
        this.$http.post("/utilization/consult/update",row)
      }
