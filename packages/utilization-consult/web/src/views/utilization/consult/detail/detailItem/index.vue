...
export default {
  ...
  methods: {
    ...
    async loadNext(isPre = false) {
      ...
      if (data.data[0].useFile){
        this.detail = data.data[0]
        this.num=data.start+1
      }else {
        this.loadNext(isPre)
        if (this.page.index==this.page.total||this.page.index==1){
          if (this.num==0){
            this.num=this.page.total
          }
          this.page.index=this.num
        }
      }
    ...
    async loadFormData() {
      ...
      this.$route.query.formDefinitionId=this.detail.formDefinitionId
    },
    async $init() {
