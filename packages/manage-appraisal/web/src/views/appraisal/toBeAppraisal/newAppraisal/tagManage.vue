<template>
  <el-card>
    <el-tabs v-model="showPane">
      <el-tab-pane label="鉴定" name='one'>
        <do-appraisal v-on:next="next" v-on:reload="reload" :form-data-id="formData.id"
                      :form-definition-id="container.formDefinitionId" :key="key"></do-appraisal>
      </el-tab-pane>
      <el-tab-pane label="鉴定记录" name='two'>
        <appraisal-history :form-data-id="formData.id"
                           :form-definition-id="container.formDefinitionId"></appraisal-history>
      </el-tab-pane>
      <el-tab-pane label="利用记录" name='utilize'>
        <utilize-history :form-data-id="formData.id" :form-definition-id="container.formDefinitionId">
        </utilize-history>
      </el-tab-pane>
      <el-tab-pane label="关键词推荐" name='three'>
...
import utilizeHistory from "../utilizeHistory";


export default {
  name: "tagManage",
  extends: abstractArchiveDetail,
  components: {addKeyWordForm, appraisalHistory, doAppraisal, utilizeHistory},
  data() {
    return {
      dynamicTags: [],
      inputVisible: false,
