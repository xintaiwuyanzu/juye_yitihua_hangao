<template>
  <section>
    <el-table :data="data" :border="true">
      ...
      <el-table-column prop="specialName" width="200" label="专题名称" align="center">
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            <p>条件: {{ scope.row.specialRemarks }}</p>
      ...
      <!--      <el-table-column prop="specialRemarks" label="描述" align="center"></el-table-column>-->
      <el-table-column prop="specialTagNames" label="专题标签" align="center"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    loadData(param) {
      ...
      this.$http.post("/appraisalSpecial/page", Object.assign(this.page, param, {rulesId: this.rulesId}))
          .then(({data}) => {
            if (data.success) {
    ...
    async remove(row) {
      ...
      this.$http.post("/appraisalSpecial/delete", {id: row.id})
          .then(({data}) => {
            if (data.success) {
    ...
    addSpecial() {
      this.$refs.editForm.form = {rulesId: this.rulesId}
      this.$refs.editForm.condition = []
