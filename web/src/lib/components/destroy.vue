...
<!--    <el-button type="primary" @click="showDialog">销毁</el-button>
    ...
    <el-dropdown
      ...
      <el-button class="search-btn" type="primary">档案销毁<i class="el-icon-arrow-down el-icon--right"/></el-button>
      <el-dropdown-menu slot="dropdown">
        <el-dropdown-item v-if="currentSelect.length>0" command="select">销毁选中</el-dropdown-item>
        <el-dropdown-item command="all">销毁所有</el-dropdown-item>
        <el-dropdown-item command="query">销毁查询</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>

    <el-dialog width="40%" title="档案销毁"
      ...
      <el-form :model="form" ref="form" :rules="rules" label-width="200px">
        ...
        <el-form-item label="销毁原因" prop="destoryOpinion" style="margin-top: 7px">
          <el-input
              type="textarea"
              :rows="2"
              style="width: 80%"
  ...
  methods: {
    ...
    async preSubmit() {
      ...
      this.$refs.form.validate(valid => {
        if(valid){
          const query = this.eventBus.getQueryByQueryType(this.sendType)
          this.$http.post('/destory/addBatch', {
