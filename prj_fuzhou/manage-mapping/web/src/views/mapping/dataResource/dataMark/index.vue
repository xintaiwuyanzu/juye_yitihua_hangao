<template>
  <section>
    <nac-info back title="元数据标注">
      <span v-show="dataResource.status==='0'">
      ...
      <span v-show="dataResource.status==='1'">
        <el-button :disabled="c.indexOf('source')===-1||c.indexOf('target')===-1||objRelation.relation===''"
                   @click="submitRelationResult"
                   type="primary">保存关系</el-button>
      </span>
      <el-button @click="drawer=true" type="primary">查看元数据方案</el-button>
    </nac-info>
    <el-card>
      <div class="left">
        <el-table :data="form.fields" size="medium" stripe style="width: 43%;height: 800px;overflow-y: scroll">
          <el-table-column label="字段" prop="fieldCode"></el-table-column>
          <el-table-column label="含义" prop="label">
            <template slot-scope="scope">
              <span>{{scope.row.label}}</span>
              <el-tag style="margin-left: 5px" v-if="markMap.has(scope.row.fieldCode)&&dataResource.status==='1'">
                {{'对象'+markMap.get(scope.row.fieldCode)}}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column
              align="center">
            <template slot="header" slot-scope="scope" v-if="dataResource.status==='0'">
              <span>当前操作对象：</span>
              <el-select clearable v-model="currentObjId">
                <el-option :key="index" :label="i.formName" :value="i.id" v-for="(i,index) in classes"></el-option>
              </el-select>
            </template>
            <template slot-scope="scope">
              <el-select clearable v-if="dataResource.status==='0'" v-model="properties[scope.$index]">
              ...
              <el-select clearable v-if="dataResource.status==='1'" v-model="c[scope.$index]">
        ...
        <div class="right">
          <el-card class="right_card">
            <div v-if="dataResource.status==='1'">
              <h4>
            ...
            <div>
              ...
              <div :key="ii" class="case" v-for="(i,ii) in caseData">
                <el-select class="i" clearable filterable placeholder="源属性" style="width: 200px" v-model="i.name">
                  <el-option :key="index" :label="i.label" :value="i.fieldCode"
                             v-for="(i,index) in form.fields"></el-option>
                </el-select>
                ...
                <div>
                  <i @click="addCase" class="el-icon-plus icon"
                     v-if="ii===0"></i>
                  <i @click="deleteCase(index)" class="el-icon-minus icon"
                     v-else></i>
                </div>
              </div>
            </div>
            ...
            <div v-if="dataResource.status==='1'">
              ...
              <el-table :data="tripleData" border height="500" max-height="500" stripe>
    ...
    <task-drawer :drawer="drawer" :formId="formId" :type="dataResource.status" @close="close"
                 ref="taskDrawer"></task-drawer>
  </section>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        //关系梳理对象
        objIds: {baseId: this.$route.query.formId, sourceId: '', targetId: ''},
        //已梳理关系
        tripleData: [],
        //对象索引标记
        markMap: new Map(),
    ...
    methods: {
      ...
      //数据来源信息（dataResource）
      async detail() {
        const {data} = await this.$post('dataResource/page', {formId: this.$route.query.formId})
        if (data && data.success) {
          this.dataResource = data.data.data[0]
          const result = this.dataResource.markResult
          if (result !== '') {
      ...
      //获取元数据库的信息
      async getFormDataId() {
        const {data} = await this.$post('dataResource/mark', {formId: this.formId})
        if (data && data.success) {
          this.form = data.data
          this.detail()
        }
      ...
      async submitMarkResult() {
        ...
        this.properties.forEach((i, index) => {
          if (i !== '') {
            properties.push({from: this.form.fields[index].fieldCode, to: i})
          }
        })
        let result = {
          properties,
          condition: this.caseData,
          objIds: {baseId: this.$route.query.formId, sourceId: this.currentObjId, targetId: ''}
        }
        const {data} = await this.$post('dataResource/transform', {result: JSON.stringify(result)})
        if (data && data.success) {
          if (data.data === -1) {
            this.$message.success('数据梳理已进入等待！')
          } else {
            if (data.data > 0) {
              this.$message.success('数据梳理成功！')
            } else {
              this.$message.error('无数据可梳理！')
            }
          }
          this.currentObjId = ''
          this.properties = []
      ...
      async submitRelationResult() {
        this.c.forEach((i, index) => {
          if (i !== '') {
            if (i === 'source') {
              this.objRelation.source = this.form.fields[index].fieldCode
            }
            if (i === 'target') {
              this.objRelation.target = this.form.fields[index].fieldCode
            }
          }
        })
        let result = {
        ...
        const {data} = await this.$post('dataResource/saveRelationResult', {result: JSON.stringify(result)})
        if (data && data.success) {
          if (data.data === -1) {
            this.$message.success('关系梳理已进入等待！')
          } else {
            if (data.data > 0) {
              this.$message.success('关系数据梳理成功')
              this.getTripleData()
            } else {
              this.$message.error('无关系可梳理！')
            }
          }
          this.c = []
          this.objRelation = {relation: ''}
          this.objIds = {baseId: this.$route.query.formId}
          this.caseData = [{name: '', when: '', value: ''}]
        }
      },
      //完成数据标注
      async finishMark() {
        this.$confirm('数据标注完成将进入关系梳理，不再重新标记数据?', '提示', {
          ...
          let obj = this.dataResource
          obj.status = '1'
          const {data} = await this.$post('dataResource/update', obj)
          if (data && data.success) {
            this.$message({
              type: 'success',
              message: '元数据标注完成!'
      ...
      //获取已梳理关系
      async getTripleData() {
        const {data} = await this.$post('tripletData/getMarkedRelations', {baseFormId: this.$route.query.formId})
        if (data && data.success) {
