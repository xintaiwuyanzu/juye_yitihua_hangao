        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-button
                  v-if="scope.row.status=='未审核'||scope.row.status=='驳回'"
...
export default {
  ...
  methods: {
    ...
    change(row) {
      this.optype = 'edit'
      this.$router.push({path: '/onlineAS/addDetail', query: {id: row.id, optype: this.optype}})
    },
    change1(row) {
      this.optype = 'edit'
      this.$router.push({path: '/onlineAS/check', query: {id: row.id}})
    },
